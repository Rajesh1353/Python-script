...
def get_statistics():
    ...
    #top_command = subprocess.run(['top', '-l 1', '-n 0'], stdout=subprocess.PIPE, shell=True).stdout.decode('utf-8').split('\n')
    ...
    cpu_load = [x / os.cpu_count() * 100 for x in psutil.getloadavg()][-1]
    statistics['cpu_load'] = round(cpu_load)

    # Memory usage
    #total_ram = subprocess.run(['sysctl', 'hw.memsize'], stdout=subprocess.PIPE, shell=True).stdout.decode('utf-8')
    total_ram = psutil.virtual_memory()
    '''#vm = subprocess.Popen(['vm_stat'], stdout=subprocess.PIPE, shell=True).communicate()[0].decode('utf-8')
    vm = 
    vmLines = vm.split('\n')
    ...
    statistics['ram'] = dict({
        'total_ram': round(total_ram[0] / 1024 ** 3, 1),
        'used_ram': round(total_ram[-2] / 1024 ** 3, 1),
    })
    ...
    """
    ping_result = subprocess.run(['ping', '8.8.8.8'], stdout=subprocess.PIPE, shell=True).stdout.decode('utf-8').split('\n')
    
    #import pdb; pdb.set_trace()

    results = ping_result[-2].strip().split(',')
    statistics['network_latency'] = dict(
        {
            'min': results[0].split('=')[-1].strip(),
            'avg': results[2].split('=')[-1].strip(),
            'max': results[1].split('=')[-1].strip(),
        }
    )
    return statistics
